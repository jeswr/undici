...
// Safe HTTP methods whose request carries no semantically significant body, so
// a body-blind cache/deduplication key (origin + method + path + headers) is
// correct. QUERY is safe and idempotent (RFC 10008) but its request content
// *is* the query, and RFC 10008 Section 2.7 requires a QUERY cache key to
// incorporate the request content. Until undici's cache key is body-aware,
// QUERY is intentionally excluded here so it cannot be cached or deduplicated
// by a key that ignores its body. See nodejs/undici#5454 and nodejs/undici#5459.
const cacheableSafeMethods = Object.freeze(['GET', 'HEAD', 'OPTIONS', 'TRACE'])
...
function assertCacheMethods (methods, name = 'CacheMethods') {
  ...
  for (const method of methods) {
    if (method === 'QUERY') {
      // QUERY is safe (RFC 10008) but its response is determined by the request
      // content (RFC 10008 Section 2.7). undici's cache key does not yet
      // incorporate the request body, so caching QUERY by a body-blind key
      // could return a response generated for a different query.
      throw new TypeError(`${name} does not support 'QUERY': a QUERY response is determined by the request content (RFC 10008, Section 2.7), which undici's cache key does not incorporate, so caching QUERY by a body-blind key could return a response for a different query`)
    }
    if (!cacheableSafeMethods.includes(method)) {
      throw new TypeError(`element of ${name}-array needs to be one of following values: ${cacheableSafeMethods.join(', ')}, got ${method}`)
    }
  }
}
