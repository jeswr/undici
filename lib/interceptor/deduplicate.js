...
const { normalizeHeaders, makeCacheKey, makeDeduplicationKey, cacheableSafeMethods } = require('../util/cache.js')
...
module.exports = (opts = {}) => {
  ...
  for (const method of methods) {
    if (method === 'QUERY') {
      // QUERY is safe (RFC 10008) but its identity is its request content
      // (RFC 10008 Section 2.7). undici's deduplication key does not incorporate
      // the request body, so deduplicating QUERY by a body-blind key could share
      // one response across concurrent requests carrying different queries.
      throw new TypeError('expected opts.methods not to include QUERY: a QUERY request is identified by its content (RFC 10008, Section 2.7), which the deduplication key does not incorporate, so deduplicating QUERY by a body-blind key could share one response across different queries')
    }
