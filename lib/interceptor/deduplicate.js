...
const { normalizeHeaders, makeCacheKey, makeDeduplicationKey, isCacheableRequestBody, bodySignificantMethods } = require('../util/cache.js')
...
module.exports = (opts = {}) => {
  ...
  return dispatch => {
    return (opts, handler) => {
      ...
      // (e.g. a stream), we cannot build a body-aware key, so dispatch
      // independently rather than risk sharing one response across different
      // queries.
