...
const { assertCacheStore, assertCacheMethods, makeCacheKey, normalizeHeaders, parseCacheControlHeader, isCacheableRequestBody, isStreamingRequestBody, readRequestBodyForKey, bodySignificantMethods, DEFAULT_MAX_REQUEST_BODY_KEY_SIZE } = require('../util/cache.js')
...
module.exports = (opts = {}) => {
  ...
  return dispatch => {
    return (opts, handler) => {
      ...
      // Computes the cache key for `effectiveOpts` (whose body, for QUERY, is now
      // an in-memory Buffer/string) and serves from / writes to the store.
      const proceedWithCache = (effectiveOpts) => {
        /**
         * @type {import('../../types/cache-interceptor.d.ts').default.CacheKey}
         */
        const cacheKey = makeCacheKey(effectiveOpts)
        const result = store.get(cacheKey)

        if (result && typeof result.then === 'function') {
          return result
            .then(result => handleResult(dispatch, globalOpts, cacheKey, handler, effectiveOpts, reqCacheControl, result))
        }
        return handleResult(dispatch, globalOpts, cacheKey, handler, effectiveOpts, reqCacheControl, result)
      }

      // A body-significant method (QUERY) is identified by its request content
      // (RFC 10008 Section 2.7), so the body must be incorporated into the key.
      if (bodySignificantMethods.has(opts.method)) {
        if (isStreamingRequestBody(opts.body)) {
          // Buffer the stream up to the cap to build a body-aware key without
          // unbounded memory: <= cap is cached; > cap is forwarded uncached.
          return readRequestBodyForKey(opts.body, maxRequestBodyKeySize).then(prepared => {
            ...
            return proceedWithCache({ ...opts, body: prepared.buffered })
          })
        }
        if (!isCacheableRequestBody(opts.body)) {
          // Body we can neither hash nor stream-buffer (e.g. Blob/FormData): bypass.
          return dispatch(opts, handler)
        }
      }

      return proceedWithCache(opts)
    }
  }
}
