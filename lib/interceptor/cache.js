...
const { assertCacheStore, assertCacheMethods, makeCacheKey, normalizeHeaders, parseCacheControlHeader, isCacheableRequestBody, bodySignificantMethods } = require('../util/cache.js')
...
module.exports = (opts = {}) => {
  ...
  return dispatch => {
    return (opts, handler) => {
      ...
      // (RFC 10008 Section 2.7). If the body cannot be hashed synchronously
      // (e.g. a stream), we cannot build a body-aware key, so bypass the cache
      // rather than risk returning a response generated for a different query.
